...
Checkpoint1     LDR   R0, R5, #4    ; Load parameter N into a register
                ...
                PUSH R2 		; Step 1: push parameter N - 2
                JSR Factorial		; Step 2: call Factorial(N - 1)
		LDR R2,R0,#0		; Step 13: retrieve return value into R2
                ADD R6,R6,#2		; Step 14: remove parameter from stack
...
MultiplyLoop    ADD   R3, R3, R2    ; Notice that by this point, R0 > 1
                ...
                STR R0,R3,#1		; Make return value = N * Factorial(N - 1)
...
Finish          ADD R6,R5,#1		; Step 9: remove local variable from stack
...
Main            LD    R6, Stack     ; Initialize stack pointer
		...
		JSR Factorial		; Step 2: call Factorial(Param)
                LDR R0,R6,#0	;verify values		; Step 13: retrieve return value
                ADD R6,R6,#2	;verify values		; Step 14: remove parameter from stack
